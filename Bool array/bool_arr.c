...
{
    vb->arr = calloc(1u, sizeof(*vb->arr));
...
uint get(const vector_bool vb, uint index)
{
    ...
    // (*a & (1u << b)) gets wheteher the bit is on or off
    // (above expr) >> b shifts the bit to the Least significant position to yield either 0 or 1u
    return ((*byte_ind & (1u << bit_idx)) >> bit_idx);
}
...
void set(vector_bool *vb, uint index, uint bool_value)
{
    ...
    // Generalise bool value to 0u or 1u
    if(bool_value)
        bool_value = 1u;
    ...
    // Toggle bit to 1u if bool value is true
    if(bool_value)
        *elem ^= 1u << bit_idx;
}
...
void expand(vector_bool *vb)
{
    ...
    uint *new_arr = realloc(vb->arr, (arr_count + 1u) * sizeof(*new_arr));
